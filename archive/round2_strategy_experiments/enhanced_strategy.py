...
class EnhancedEuromillionsStrategy:
    """
    Enhanced Euromillions strategy based on May 6 results analysis
    """
    ...
    def get_number_frequency(self, recent_weight=0.7):
        ...
        num_freq, star_freq = self.stats.get_number_frequency()
        
        # Apply recency boost - more recent draws get higher weight
        draws = self.data.copy()
        if len(draws) > 0:
            ...
            # Calculate days since most recent draw for weighting
            if 'date' in draws.columns and len(draws) > 0:
                most_recent = draws['date'].max()
                draws['days_ago'] = (most_recent - draws['date']).dt.days
                max_days = draws['days_ago'].max()
                draws['recency_weight'] = 1 - (recent_weight * draws['days_ago'] / max_days)
                
                # Recalculate weighted frequencies
    ...
    def get_correlation_adjusted_scores(self):
        ...
        hot_cold = self.stats.get_hot_cold_numbers(periods=[10, 20, 50])
        
        # Calculate correlation scores from past draws
        num_corr = self.stats.get_number_correlations()
        star_corr = self.stats.get_star_correlations()
        ...
        for num in range(1, 51):
            ...
            for recent_num in self.recent_draw['numbers']:
                if (num, recent_num) in num_corr or (recent_num, num) in num_corr:
                    corr_factor = num_corr.get((num, recent_num), 0) or num_corr.get((recent_num, num), 0)
        ...
        for star in range(1, 13):
            ...
            for recent_star in self.recent_draw['stars']:
                if (star, recent_star) in star_corr or (recent_star, star) in star_corr:
                    corr_factor = star_corr.get((star, recent_star), 0) or star_corr.get((recent_star, star), 0)
    ...
    def apply_frequency_strategy(self, recent_weight=0.7):
        ...
        if remaining_count > 0:
            ...
            remaining_picks = np.random.choice(
                [nums[i] for i in remaining_indices],
                size=min(remaining_count, len(remaining_indices)),
                replace=False,
                p=remaining_weights
            )
            selected_numbers.extend(remaining_picks)
        ...
        if remaining_count > 0:
            ...
            if remaining_indices:
                ...
                remaining_picks = np.random.choice(
                    [stars[i] for i in remaining_indices],
                    size=min(remaining_count, len(remaining_indices)),
                    replace=False,
                    p=remaining_weights
                )
                selected_stars.extend(remaining_picks)
...
def main():
    ...
    strategy = EnhancedEuromillionsStrategy()
